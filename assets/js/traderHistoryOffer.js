    ...
    API_REQUEST(url, "GET").then(function (res) {
        ...
        if (res.status.code === 200) {
            ...
            for (var i = 0; i < Object.keys(res.content).length; ++i) {
                ...
                var color = (res.content[i]['status'] == 6) ? "primary" : res.content[i]['status'] == 5 ? "secondary" : "primary";
                var buttonText = res.content[i]['status'] == 6 ? "Ok" : res.content[i]['status'] == 5 ? "deny" : "ok";
